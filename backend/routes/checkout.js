...
router.post("/checkout_order_details", (req, res) => {
  ...
  const insertQuery =
    "INSERT INTO OrderDetails (UserID, Country, Address, Landmark, TownCity, State, Zip) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  connection.query(insertQuery, values, (error, results) => {
    if (error) {
      ...
      res
        .status(500)
        .json({ error: "An error occurred while inserting order details" });
    } else {
...
router.get("/get/checkout_order_details/:userId", async (req, res) => {
  const { userId } = req.params;

  const getQuery = "SELECT * FROM  OrderDetails WHERE UserId = ?";
  const value = [userId];

  connection.query(getQuery, value, (error, results) => {
    if (error) {
      console.log(error);
      console.error("Error inserting order details:", error);
      res
      ...
      if (results.length > 0) {
        ...
        res.status(200).json({ success: false, message: "No order details found for the user" });
      }
    }
  });
});
